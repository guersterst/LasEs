...
\begin{samepage}
	...
	\ftable{
		\fentry{add-reviewer-iTxt}{<h:inputText>}{Eingabefeld zur Angabe einer E-Mail-Adresse}{value="\#{}" \newline title="\#{tooltip.reviewerEmail}}{render="\#{}" nur für E und A}{emailAddressExistsValidator \newline required="true" \newline pattern="([^.@]+)(\.[^.@]+)*@([^.@]+\.)+([^.@]+)"}
		
		\fentry{add-deadline-reviewer-iTxt}{<h:inputText>}{Eingabefeld zur Angabe einer Deadline.}{value="\#{}" \newline title="\#{tooltip.reviewerDeadline}"}{render="\#{}"für A und E}{converterDateTime \newline patter="dd/MM/yyyy"}
		
		\fentry{add-reviewer-cbtn}{<h:commandButton>}{Knopf zum Hinzufügen des Gutachters}{value="\#{label.addReviewer}" \newline action="\#{}" \newline title="\#{tooltip.addReviewer}"}{render="\#{}" nur für E und A}{/}
