...
public class ScientificForumService implements Serializable {
    ...
    public void change(ScientificForum newForum) {
        ...
        try {
            ...
            logger.severe(e.getMessage() + "caused the change operation to fail for: " + newForum.getId());
            uiMessageEvent.fire(new UIMessage(message.getString("dataNotFound"), MessageCategory.ERROR));
        } catch (DataNotWrittenException e) {

            logger.severe("A database error occurred and the operation could not be performed.");
            uiMessageEvent.fire(new UIMessage(message.getString("dataNotWritten"), MessageCategory.ERROR));
        } catch (KeyExistsException e) {

            logger.warning("It was attempted to create a forum with an existing name: " + newForum.getName());
    ...
    public void remove(ScientificForum forum) {
        ...
        try {
            ...
            logger.severe(e.getMessage() + "caused the removal operation to fail for: " + forum.getId());
            uiMessageEvent.fire(new UIMessage(message.getString("dataNotFound"), MessageCategory.ERROR));
        } catch (DataNotWrittenException e) {
    ...
    public void addEditor(User editor, ScientificForum forum) {
        ...
        try {
            ...
            logger.warning("Could not query whether this editor is a duplicate.");
        }
    ...
    public void addScienceField(ScienceField scienceField, ScientificForum forum) {
        ...
        try {
            ...
            logger.warning("Could not query whether this field is a duplicate: " + scienceField.getName());
        }
        try {
